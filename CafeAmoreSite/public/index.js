...
import { collection, getDocs, doc, setDoc, getDoc, deleteDoc } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
import { getAuth, onAuthStateChanged } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-auth.js";
import { addToCart } from './cart.js';
// ==========================
// Modular Toast Notification
// ==========================
function showToast(message = "Item added!", duration = 2000) {
...
// ==========================
// LOAD PRODUCTS (Grouped by Subcategory, Horizontal)
// ==========================
  ...
  try {
    ...
    for (const mainCat in grouped) {
      ...
      for (const subCat in grouped[mainCat]) {
        ...
        // Horizontal scroll container with flex styling
        const horizontalContainer = document.createElement('div');
        horizontalContainer.className = 'subcategory-products horizontal-scroll';
        Object.assign(horizontalContainer.style, {
          display: 'flex',
          flexWrap: 'nowrap',
          gap: '15px',
          overflowX: 'auto',
        ...
        for (const product of grouped[mainCat][subCat]) {
          ...
          if (product.sizes?.length) displayPrice = Math.min(...product.sizes.map(s => s.price || Infinity));

          const isUnavailable = !product.available || product.sizes?.every(s => s.qty <= 0);
          if (isUnavailable) card.classList.add('unavailable');

          const imgHTML = product.image
            ? `<img src="${product.image}" alt="${product.name}" style="width:100%; border-radius:10px; margin-bottom:10px; margin-top:20px;">`
            : '';
          ...
          if (!isUnavailable && addBtn) addBtn.addEventListener('click', () => openCartPopup(product));
...
function openCartPopup(product) {
  ...
  if (Array.isArray(product.sizes) && product.sizes.length) {
    const sizeLabelHeading = document.createElement('p');
    sizeLabelHeading.textContent = 'Sizes:';
    sizesContainer.appendChild(sizeLabelHeading);

      product.sizes.forEach((size, index) => {
      const label = document.createElement('label');
      label.classList.add('size-btn');
      label.textContent = `${size.name} - ₱${size.price.toFixed(2)}`;
  ...
  if (Array.isArray(product.addons) && product.addons.length) {
    const addonLabelHeading = document.createElement('p');
    addonLabelHeading.textContent = 'Add-ons:';
    addonsContainer.appendChild(addonLabelHeading);
  ...
  confirmBtn.onclick = () => {
    ...
    const sizeToPass = selectedSize
      ? { id: selectedSize.id, name: selectedSize.name, price: Number(selectedSize.price || 0) }
      : null;
...
cartPopup.querySelector('.close-cart').addEventListener('click', () => {
  cartPopup.style.display = 'none';
});
