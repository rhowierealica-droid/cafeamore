...
function listenOrders() {
  ...
  unsubscribeOrders = onSnapshot(collection(db, "DeliveryOrders"), snapshot => {
    ...
    filteredOrders.forEach(order => {
      ...
      orderHeader.appendChild(toggleBtn);
      ...
      order.items?.forEach(p => {
        ...
        const productName = p.product || p.name || "Matcha";
        const sizeText = p.size ? `${p.size} - ₱${p.sizePrice}` : `₱${p.sizePrice || 0}`;
        const addonsTotal = p.addonsPrice || 0;
        const productPrice = ((p.sizePrice || 0) + addonsTotal) * qty;
...
function openFeedbackModal(orderId, items) {
  ...
  items.forEach((item, index) => {
    if (!item) return;
    const productName = item.product || item.name || "Matcha";
  ...
  form.onsubmit = async (e) => {
    e.preventDefault();
    const feedbacks = [];
    form.querySelectorAll("textarea").forEach(t => feedbacks.push(t.value));
    try {
      await updateDoc(doc(db, "DeliveryOrders", orderId), { feedback: arrayUnion(...feedbacks) });
