...
messagePopup.style.background = "#f44336";
messagePopup.style.color = "#fff";
messagePopup.style.padding = "10px 20px";
messagePopup.style.borderRadius = "5px";
messagePopup.style.boxShadow = "0 2px 10px rgba(0,0,0,0.2)";
...
deletePopup.innerHTML = `
  <div class="popup-content" style="max-width: 400px;">
    ...
    <div style="text-align: center; margin-top: 15px;">
      <button id="confirmDeleteBtn" style="background-color: #f44336; color: white;">Delete</button>
...
popupForm.addEventListener("submit", async e => {
  ...
  try {
    if (editingProductId) {
      await updateDoc(doc(db, "products", editingProductId), { name, category, description, ingredients, sizes, addons, others });
    } else {
      await addDoc(collection(db, "products"), { name, category, description, ingredients, sizes, addons, others, available: true, createdAt: serverTimestamp() });
    }
    closePopup();
  } catch (err) {
    showPopupMessage("Error: " + err.message);
